...
=head1 FUNCTIONS
   ...
   The following operators are overloaded:
    ...
    NOTE: Overloading of '**' and '**=' accepts only a positive integer
          ...
          (The GMP library does not provide an mpq_pow() function.)
    ...
    For general compatibility with perl in general and Math::BigRat in
   ...
   '<<' operators has now been added.
    ...
    YMMV with older versions of Math::MPFR but, if you use Math-MPFR-4.35
    ...
    Math::MPFR object (calculated using default Math::MPFR precision and
    default Math::MPFR rounding mode).
    ...
    TODO - enable Math::GMPq overloading to handle these comparisons.
