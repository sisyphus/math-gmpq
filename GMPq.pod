...
=head1 FUNCTIONS
   ...
   Overloading occurs with numbers, strings (bases 2, 8,10 & 16 only),
   Math::GMPq objects, Math::GMPz objects, Math::GMP objects and
   Math::MPFR objects. Strings are first converted to Math::GMPq
   objects, then canonicalized.
   The following operators are overloaded:
    ...
    Overloading of '**' and '**=' accepts only positive integer  values -
    in the form of a string, an IV, a Math::MPFR object, a  Math::GMPz, a
    Math::GMP object or a Math::GMPq object. An exception will be raised
    unless the value of the Math::GMPz/Math::GMP/Math::GMPq object fits
    into an unsigned int. (Math::MPFR object values are allowed to be
    negative and/or fractional as the '**' and '**=' operations return a
    Math::MPFR object.)
   ...
   '<<' operators has now been added.
    ...
    When overloading operations that involve Math::GMP and Math::GMPq
    objects, a Math::GMPq object is always returned. You will also need
    to load the Math::GMP_OLOAD module for operations that call
    Math::GMP's overloading subs.
    For such operations that instead call Math::GMPq's overloading subs,
    then Math::GMP_OLOAD is not needed.
