...
=head1 FUNCTIONS
   ...
   though often they will be distinct perl variables referencing
   ...
   Overloading occurs with IVs, strings (bases 2, 8,10 & 16 only), NVs,
   Math::GMPq(v0.68) objects, Math::GMPz(v0.69) objects, Math::GMP(v2.11)
   objects and Math::MPFR (v4.19) objects.
   ...
   The following operators are overloaded:
    ...
    Let $LHS' and '$RHS' denote a variable that is any one of those data
    types just specified; let '$MPQ' denote a Math::GMPq object; let 'OP'
    denote any one of the '+', '-', '*', '/' and '%' arithmetic operators.
    Then, so long as one (or both) of $LHS and $RHS is a Math::GMPq
    object we can do the following:

      $RET = $LHS OP $RHS;

      If $LHS is a Math::MPFR object, then so too is $RET. otherwise
      $RET is a Math::GMPq object.
      NOTE: If $LHS is a Math::GMP object then our code needs to also:
            use Math::GMP_OLOAD;

    Let 'OP=' denote any one of the '+=', '-=', '*=', '/=' and '%='
    operators. Then we can do:

      $MPQ OP= $RHS;

      Again, if $RHS is a Math::MPFR object, then a Math::MPFR object
      is returned - with $MPQ changing to a Math::MPFR object.

    And similarly for the '**' and '**=' operators - except that,
    unless $LHS is a Math::MPFR object, $RHS must be neither a
    Math::GMPq object nor an NV, and must hold a non-negative integer
    value that fits into an unsigned int.

    Let 'CMP' denote  any one of the overloaded '==', '!=', '<', '<=',
    '>', '>=', '<=>' comparison operators. We can test the condition:

      ($LHS CMP $RHS)

      with the only caveat being that, if $LHS is a Math::GMP object,
      we need to:
         use Math::GMP_OLOAD;
    ...
    When the 'OP' operations return a Math::MPFR object, the precision
    of that Math::MPFR object will be the same as the Math::MPFR object
    that was provided as an argument.
    For the 'OP=' operations, the returned Math::MPFR object will have
    current Math::MPFR default precision.
    When the '%', '%=', '**' and '**=' operations will return a Math::MPFR
    object, the  value of the Math::GMPq argument is first set to a
    Math::MPFR object using the current Math::MPFR default precision and
    default rounding mode.

    Comparisons between Math::GMPq and Math::GMPz objects can be performed
    using either the Math::GMPz/Math::GMP or Math::GMPq overloading.
    For example, ($mpq > $mpz) is equivalent to ($mpz < $mpq).

    Let 'BITWISE' denote any one of the overloaded '&', '|' and '^'
    operations, introduced for compatibility with Math::BigRat. We can do:

      $RET = $GMP BITWISE $RHS; # $RHS excludes Math::MPFR here.

      which will firstly convert $RHS to a Math::GMPq object (if it's not
      already a Math::GMPq object). $RET is a Math::GMPq object.
      The converse operation of:

      $RET = $LHS BITWISE $GMP;

      will likely (not tested) fail.

    Let 'SHIFT' denote either of '<<' or '>>', also introduced for
    compatibility with Math::BigRat:

      $RET = $MPQ SHIFT $perlnumber; # $RET is a Math::GMPq object.

    The first argument recieved by the Math::GMPq overloading subroutines
    is always the Math::GMPq object. To determine the type of the second
    argument, these subroutines work through the following steps, using
    the first value it finds, or croaking if it gets to step 6:

    1. If the variable is a UV then that value is used. The variable
       is considered to be a UV if the IOK and IsUV flags are set.

    2. If the variable is an IV (IOK but not IsUIV), then the IV value
       is used.
    ...
    4. If the variable is an NV, then that value is used. The
       ...
       NOTE: Providing an NV to the overloaded '**' and '**='
             subroutines currently generates an error.
   ...
   $bool = Rmpq_integer_p($op);
    ...
    Else returns false.
    The mpq_integer_p function is not implemented in gmp.

   $bool = Rmpq_fits_uint_p($op);
    Returns true if $op >= 0 && $op holds an integer value that
    fits into an unsigned int. Else returns false.
    The mpq_fits_uint_p function is not implemented in gmp.

   Rmpq_pow_ui($rop, $op, $ui);
    Sets $rop top the value of $op ** $ui.
    The mpq_pow_ui function is not implemented in gmp.
...
=head1 LICENSE
    ...
    Copyright 2006-2011, 2013-26, Sisyphus
