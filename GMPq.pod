...
=head1 FUNCTIONS
   ...
   $rop = new Math::GMPq($arg);
    ...
    Beginning with version 0.65, new(), along with the Rmpq_init_set_str
    & Rmpq_set_str functions, and the operator-overloading functions all
    ...
    Eg. we handle the following, though GMP's mpq_set_str() does not:
     ...
     new('F@10', 16);    # same as '0xFp+64' (276701161105643274240);
                         # the exponent is read as 0x10 (== 16).
     new('F@16', -16);   # same as '0xFp+64' (276701161105643274240);
                         # the exponent is read as the decimal 16.
     new('12@4', 61);    # 872287983
     new('-12@-5', 61)   # 64/844596301
     And same for Rmpq_init_set_str and Rmpq_set_str.
     Note: When using the "mantissa@exponent" notation, both "mantissa"
     (aka significand) and "exponent" must be integers, and must be
     expressed in their full integer form (ie not in scientific notation).
     Specifying a negative value for the base indicates that the exponent
    ...
    bases are therefore limited to 16 (via 0x prefix), 8 (via 0o prefix),
    2 (via 0b prefix) and 10 (no prefix).

   $rop = Rmpq_init_set_str($arg, $base);
    As for new() but a second arg must be provided.
   ...
   formats. The following operators are overloaded:
   ...
   '<<' operators has now been added.
    ...
    or 'Math::MPFR'/'Math::GMPz'/'Math::GMP' will not work.
    ...
    default precision and the current Math::MPFR default rounding mode.
    For the '%', '%=', '**' and '**=' overloading, the  value of the
    Math::GMPq object is first set to a Math::MPFR object using the
    same default precision and defauilt rounding mode.
