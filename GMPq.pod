...
=head1 FUNCTIONS
   ...
   The following operators are overloaded:
    ...
    operators has now been added. (TODO - expand on this when it's all
    done.)

    Atempting to use the overloaded operators with objects that
    have been blessed into some package other than 'Math::GMPq'
    or 'Math::MPFR'/'Math::GMPz'/'Math::GMP'/'Math::MPFR' (limited
    applications) will not work.
    Math::MPFR objects can be provided as the second arg with '+',
    '-', '*', '/' and '**' operators, but will work only if Math::MPFR
    is at version 3.13 or later - in which case the operation will
    ...
    Math::GMPz and Math::GMP objects can be provided as the second
    arg with comparison operators ( == != < <= > >= <=> ) and the '+',
    '+=',  '-', '-=', '*', '*=', '/', '/=', '**' and '**=' operators.
